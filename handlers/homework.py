...
async def homework1(message: Message, state: FSMContext):
    try:
        teacher = Teacher()
        teacher.get_statistics(message.from_user.id)
        if teacher.students_id:
...
async def homework1(callback: CallbackQuery, state: FSMContext):
    try:
        await state.update_data(student_id=int(callback.data.split('_')[3]))
...
async def homework2(message: Message, state: FSMContext):
    ...
    #await state.update_data(file_name=0, file_type=0, file_data=0)


@router_homework.callback_query(F.data == 'send_file', Homework.second)
async def homework2(callback: CallbackQuery, state: FSMContext):
    try:
...
async def homework3(message: Message, state: FSMContext, bot: Bot):
    try:
...
async def homework4(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        logging.error(f"Ошибка в функции homework_set_date: {e}")
        await callback.message.answer('❌Ошибка, обратитесь в поддержку')

@router_homework.callback_query(SimpleCalendarCallback.filter(), Homework.third)
async def homework5(callback: CallbackQuery, callback_data: SimpleCalendarCallback, state: FSMContext
):
    try:
        ...
        if selected:
            ...
            await callback.answer(f"Дата установлена: {date.strftime('%d.%m.%Y')}")
            await callback.message.edit_text("Дата сдачи установлена", reply_markup=keyboard_homework.markup)
        else:
